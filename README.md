...
> 1. **1.5차 (BM25):** 로컬 알고리즘이 단어가 많이 겹치는 뻔한 중복 기사들을 API 호출 없이(무료로) 빠르게 걸러냅니다.
> 2. **2차 (Gemini API):** 1.5차를 통과한 기사들만 Gemini API로 전송하여 의미론적으로 문맥이 동일한 기사를 심층 필터링합니다. (예: "NYT sues OpenAI" vs "New York Times takes legal action against Sam Altman's company")
> 
