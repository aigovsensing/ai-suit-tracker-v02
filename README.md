...
이메일 발송 기능을 활성화하려면 깃허브 저장소 설정에서 아래의 환경 변수들을 등록해 주세요.
...
| --- | --- | --- | --- |
| `ENABLE_EMAIL_SENDER` | 선택 | `0` | `1` 설정 시 이메일 전송 기능 활성화 |
| `SMTP_HOST` | 선택 | `smtp.gmail.com` | 발송용 이메일 SMTP 서버 |
| `SMTP_PORT` | 선택 | `587` | 발송용 이메일 SMTP 포트 번호 |
| `SMTP_USER` | **필수** | (없음) | 발송용 Gmail 계정 주소 (예: `ai.gov.sensing@gmail.com`) |
| `EMAIL_FROM` | 선택 | `SMTP_USER`와 동일 | 알림 발신자 주소 |
| `EMAIL_TO` | 선택 | `data/email.json` 참조 | 알림 수신자 주소 (여러 명일 경우 쉼표 `,` 로 구분) |

*수신자 주소(`EMAIL_TO`)가 비어 있는 경우, 기본적으로 [email.json](./data/email.json) 파일 내의 `receivers` 목록을 참조하며, 이마저도 비어 있으면 발신자 본인(`SMTP_USER`)에게 발송됩니다.*
