...
## 📩 이메일 전송 (FormSubmit 연동)

본 깃허브 레포의 이메일 전송 기능은 정적 페이지 환경에서 서버리스로 이메일을 전송할 수 있는 **FormSubmit.co** 서비스를 사용합니다.

**전송 방식**: AJAX (fetch API) 비동기 POST 요청  
**API 엔드포인트**: `https://formsubmit.co/ajax/ai.gov.sensing@gmail.com`  
**주요 보안 및 관리 설정**:  
* **스팸 방지 (Honeypot)**: 봇의 자동 제출을 차단하기 위해 숨겨진 필드 `_honey` 사용  
* **캡차 비활성화 (_captcha: false)**: 백그라운드 AJAX 전송을 방해하지 않도록 캡차 비활성화  
* **이메일 제목 커스텀 (_subject)**: `[ai.gov.sensing] "(조간뉴스) 2일간의 소송센싱 주요 동향 현황"` 또는 `"(석간뉴스) 당일 신규/업데이트 소송건 요약 보고서 (Gemini)"`  

**최초 활성화 방법 (신규 수신 주소 등록 시 필수)**:  
FormSubmit.co는 신규 이메일 주소로 첫 POST 요청이 도달하면 자동으로 활성화 인증 메일을 발송합니다. 아래 방법 중 하나를 선택하여 최초 1회 활성화를 완료해 주세요.

1. **활성화 트리거(POST) 발송 (택일)**:
   - **방법 A (권장)**: 터미널에서 아래 `curl` 명령을 실행하여 활성화 요청을 보냅니다 (대상 수신 이메일 입력).
     ```bash
      curl -H "Referer: https://github.com" -X POST https://formsubmit.co/ajax/ai.gov.sensing@gmail.com -d "message=Activation Test"
     ```
   - **방법 B**: `ENABLE_EMAIL_SENDER=1`로 설정한 후 모니터링 스크립트를 1회 실행합니다.
2. **인증 메일 확인**: 수신 이메일 주소로 전송된 **"FormSubmit.co - Form Activation"** 활성화 메일을 확인합니다.  
3. **최종 승인**: 메일 내부의 **"Activate Form"** 버튼을 클릭하면 활성화가 완료되며, 이후의 모든 리포트 제출 건은 메일함으로 즉시 수신됩니다.  
