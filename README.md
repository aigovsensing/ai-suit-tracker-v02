...
- **강력한 중복 제거 시스템**:
    ...
    *   **의미론적 중복 제거 (Semantic Dedup)**: 단순히 제목이 같은 것을 넘어, **Gemini Embedding(text-embedding-004) 기술을 활용하여 문맥상 동일한 뉴스를 다루는 기사들을 감지**하고 자동으로 필터링합니다. (예: "NYT sues OpenAI"와 "OpenAI hit with lawsuit by New York Times"를 동일 뉴스로 인식)
...
| `GEMINI_API_KEY` | (선택/Gemini 사용 시) Google AI Studio API 키 (실행당 평균 1~3회 호출) |
...
| `GEMINI_AISUIT_TREND_DAYS` | (공백) | **설정 시 Gemini 요약 기능 활성화.** 분석할 데이터의 기간(예: `3`)을 입력하면 해당 기간의 동향을 Gemini가 분석하여 발행합니다. (실행당 1회 호출) |
| `PREVIOUS_ITEM_DEDUP_DAYS` | (공백) | **설정 시 이전 날짜 이슈와 중복 체크 수행.** `3` 설정 시 설정된 일수(예: 3일) 내의 이슈 댓글들을 확인하여 중복된 소송/뉴스는 리포트에서 제외합니다. (미설정 시 이전 이슈와 중복 체크 안 함) |
| `GEMINI_SEMANTIC_DEDUP` | `0` | **설정 시 의미론적 중복 제거 활성화.** (1: 활성화, 0: 비활성화, 실행당 약 2회 Embedding API 호출) |
| `SEMANTIC_DEDUP_THRESHOLD` | `0.85` | 의미론적 중복 판정 임계값 (0.0~1.0, 높을수록 엄격) |
